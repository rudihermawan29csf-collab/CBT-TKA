...
// Updated Character List with Gaming Art & Islamic-FF Names
const CHARACTERS = [
    { id: 'alok', name: 'Alok Fatih', role: 'Support', img: 'https://images.unsplash.com/photo-1616423668352-73a7c36b85c2?q=80&w=800&auto=format&fit=crop', hp: 90, speed: 85 }, // 3D Male
    { id: 'kelly', name: 'Kelly Zahra', role: 'Rusher', img: 'https://images.unsplash.com/photo-1635860718501-c8c34f9a5676?q=80&w=800&auto=format&fit=crop', hp: 80, speed: 100 }, // 3D Female
    { id: 'hayato', name: 'Hayato Yusuf', role: 'Tank', img: 'https://images.unsplash.com/photo-1594736797933-d0501ba2fe65?q=80&w=800&auto=format&fit=crop', hp: 100, speed: 70 }, // Samurai/Cyber
    { id: 'moco', name: 'Moco Aminah', role: 'Intel', img: 'https://images.unsplash.com/photo-1579783902614-a3fb3927b6a5?q=80&w=800&auto=format&fit=crop', hp: 75, speed: 90 }, // Cyber Girl
    { id: 'wukong', name: 'Wukong Bilal', role: 'Camo', img: 'https://images.unsplash.com/photo-1615592389070-bcc97e05ad01?q=80&w=800&auto=format&fit=crop', hp: 85, speed: 85 }, // Masked
    { id: 'chrono', name: 'Chrono Rayyan', role: 'Shield', img: 'https://images.unsplash.com/photo-1620645068270-43eb51368953?q=80&w=800&auto=format&fit=crop', hp: 95, speed: 60 }, // Tech
    { id: 'maxim', name: 'Maxim Hamzah', role: 'Medic', img: 'https://images.unsplash.com/photo-1594736797933-d0501ba2fe65?q=80&w=800&auto=format&fit=crop', hp: 85, speed: 80 }, // Repeat placeholder style
    { id: 'kla', name: 'Kla Idris', role: 'Fighter', img: 'https://images.unsplash.com/photo-1563178406-4f4648c68037?q=80&w=800&auto=format&fit=crop', hp: 90, speed: 85 }, // Hooded
    { id: 'andrew', name: 'Andrew Umar', role: 'Tank', img: 'https://images.unsplash.com/photo-1605806616949-1e87b487bc2a?q=80&w=800&auto=format&fit=crop', hp: 100, speed: 65 }, // Tactical
    { id: 'miguel', name: 'Miguel Tariq', role: 'Slayer', img: 'https://images.unsplash.com/photo-1542345812-d98b5cd6cf98?q=80&w=800&auto=format&fit=crop', hp: 80, speed: 80 }, // Futuristic
];
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ student, exams, activeTab, onLogout }) => {
  // --- INTRO FLOW STATE ---
  // 0: Welcome, 1: Character Select, 2: Rules, 3: Lobby (Dashboard)
    ...
    const handleVisibilityChange = () => {
        if (document.hidden) {
            setViolationCount(prev => {
                ...
                if (newCount >= 3) {
                    // Trigger Auto Submit / Disqualification
  ...
  const handleSubmitExam = (forceDisqualify = false) => {
    ...
    if (forceDisqualify) {
        ...
        setViolationCount(3);
        setShowViolationWarning(false); // Close warning modal if open
    }
    
    setIsSubmitted(true);
  };

  // --- Render Components for Question Types ---
  ...
  // --- UPDATED COMPLEX CHOICE RENDER ---
  const renderComplexChoice = (q: Question) => {
    // Hitung jumlah jawaban benar yang dibutuhkan
    ...
    return (
        <div className="space-y-4">
            ...
            <p className="text-xs text-slate-500 italic mt-2">
                * Anda telah memilih {currentSelected} dari {neededAnswers} jawaban.
            </p>
        </div>
    );
  };
  ...
  // ----------------------------------------------------------------
  // MAIN VIEWS (INTRO FLOW, EXAM, RESULT)
  // ----------------------------------------------------------------
              ...
              <div className="relative z-10 text-center p-6 flex flex-col items-center">
                   <div className="mb-6 inline-block animate-float">
                        <img 
                          ...
                          alt="Logo" 
                          className="h-32 md:h-40 mx-auto drop-shadow-[0_0_25px_rgba(234,179,8,0.8)]" 
                        />
                   </div>
                   
                   <div className="space-y-2 mb-10">
                      <h1 className="text-5xl md:text-7xl font-black italic text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-yellow-600 tracking-tighter drop-shadow-lg uppercase animate-fade-in-up">
                         Selamat Datang
                      </h1>
                      <h2 className="text-3xl md:text-5xl font-black text-white tracking-widest uppercase animate-fade-in-up animate-delay-200">
                         {student.name}
                      </h2>
                      <div className="h-1 w-24 bg-yellow-500 mx-auto mt-4 rounded-full animate-fade-in-up animate-delay-200"></div>
                   </div>

                   <p className="text-slate-400 text-lg md:text-xl font-mono uppercase tracking-[0.3em] mb-12 animate-fade-in-up animate-delay-400">
                       CBT Battle Arena • SMPN 3 Pacet
                   </p>
                   ...
                   >
                       <span className="skew-x-[12deg] inline-block flex items-center gap-3">
                         Mulai Misi <ChevronRight size={24} className="group-hover:translate-x-1 transition-transform"/>
                       </span>
                       <div className="absolute inset-0 bg-white/20 -translate-x-full group-hover:translate-x-full transition-transform duration-500 skew-x-[12deg]"></div>
                   </button>
              </div>
          </div>
      );
  ...
  if (introStep === 1) {
      return (
          <div className="h-screen w-full bg-slate-950 flex flex-col relative overflow-hidden text-white font-sans">
               {/* Fixed Background with FF Theme */}
               <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1555680202-c86f0e12f086?q=80&w=2670&auto=format&fit=crop')] bg-cover bg-center"></div>
               <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-slate-900/80 to-black/95"></div>
               
               {/* Decorative Particles */}
               <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-yellow-500 via-red-500 to-yellow-500 animate-pulse z-20"></div>

               {/* Header - Fixed */}
               <div className="relative z-20 pt-8 pb-4 text-center shrink-0">
                   <h2 className="text-3xl md:text-4xl font-black text-white uppercase italic tracking-tighter drop-shadow-[0_0_10px_rgba(0,0,0,0.8)]">
                       <span className="text-yellow-500">SELECT</span> YOUR HERO
                   </h2>
                   <p className="text-slate-400 text-xs uppercase tracking-[0.3em] mt-1">Choose wisely, Survivor</p>
               </div>
               
               {/* Scrollable Grid Container */}
               <div className="relative z-10 flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 pb-32">
                   <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
                       {CHARACTERS.map(char => (
                           <div 
                             key={char.id}
                             onClick={() => setSelectedCharacter(char.id)}
                             className={`relative group cursor-pointer transition-all duration-300 transform hover:scale-[1.02] ${selectedCharacter === char.id ? 'z-10' : 'z-0'}`}
                           >
                               {/* Card Container */}
                               <div className={`
                                   h-[280px] md:h-[320px] w-full overflow-hidden relative 
                                   clip-path-polygon border-2 transition-all duration-300
                                   ${selectedCharacter === char.id 
                                     ? 'border-yellow-500 shadow-[0_0_25px_rgba(234,179,8,0.6)] grayscale-0 bg-yellow-900/20' 
                                     : 'border-slate-700 hover:border-white/50 grayscale hover:grayscale-0 bg-black/40'}
                               `}
                               ...
                               >
                                   ...
                                   <div className="absolute bottom-0 left-0 w-full p-4">
                                       <div className="flex justify-between items-end mb-1">
                                           <h3 className={`text-xl md:text-2xl font-black italic uppercase tracking-tighter leading-none ${selectedCharacter === char.id ? 'text-yellow-400' : 'text-white'}`}>
                                               {char.name}
                                           </h3>
                                       </div>
                                       <span className="text-[10px] uppercase font-bold bg-black/80 px-2 py-1 border border-slate-600 text-slate-300 mb-2 inline-block">
                                               {char.role}
                                       </span>
                                       
                                       {/* Fake Stats */}
                                       <div className="space-y-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                           <div className="flex items-center gap-2 text-[10px]">
                                               <Heart size={10} className="text-red-500" fill="currentColor"/>
                                               <div className="flex-1 h-1 bg-slate-700 rounded-full overflow-hidden">
                                                   <div className="h-full bg-red-500" style={{width: `${char.hp}%`}}></div>
                                               </div>
                                           </div>
                                           <div className="flex items-center gap-2 text-[10px]">
                                               <Zap size={10} className="text-yellow-500" fill="currentColor"/>
                                               <div className="flex-1 h-1 bg-slate-700 rounded-full overflow-hidden">
                                                   <div className="h-full bg-yellow-500" style={{width: `${char.speed}%`}}></div>
                                               </div>
                                           </div>
                                       </div>
                                   </div>
                                   ...
                                   {/* Selected Indicator */}
                                   {selectedCharacter === char.id && (
                                       <div className="absolute top-2 right-2 bg-yellow-500 text-black p-1 rounded-sm shadow-lg animate-pulse">
               ...
               <div className="absolute bottom-0 left-0 w-full p-4 md:p-6 bg-gradient-to-t from-black via-black/90 to-transparent z-30 flex justify-between items-center border-t border-white/5">
                   <button 
                     onClick={() => setIntroStep(0)} 
                     className="flex items-center gap-2 text-slate-400 hover:text-white uppercase font-bold text-xs tracking-widest px-4 py-2 hover:bg-white/5 transition"
                   >
                   ...
                   <button 
                     ...
                     className={`px-10 py-3 font-black uppercase tracking-widest transition clip-path-polygon flex items-center gap-2 shadow-lg
                     ${selectedCharacter 
                        ? 'bg-yellow-500 text-black hover:bg-yellow-400 hover:scale-105 cursor-pointer shadow-yellow-500/20' 
      ...
      return (
          <div className="h-screen w-full bg-slate-950 flex flex-col items-center justify-center p-6 relative overflow-hidden text-white">
              ...
              <div className="max-w-3xl w-full bg-slate-900/90 border border-white/10 p-8 md:p-12 relative shadow-2xl backdrop-blur-sm">
                  ...
                  <h2 className="text-3xl font-black text-white mb-8 text-center uppercase tracking-widest flex items-center justify-center gap-3">
                      <AlertCircle className="text-yellow-500" /> Aturan Pertempuran
                  </h2>

                  <div className="space-y-4 mb-8 text-slate-300 font-medium">
                      <div className="flex gap-4 items-start">
                          <span className="bg-slate-800 text-yellow-500 font-bold w-8 h-8 flex items-center justify-center rounded-sm shrink-0">1</span>
                          <p>Waktu ujian berjalan mundur. Jika waktu habis, jawaban tersimpan otomatis.</p>
                      </div>
                      <div className="flex gap-4 items-start">
                          <span className="bg-slate-800 text-yellow-500 font-bold w-8 h-8 flex items-center justify-center rounded-sm shrink-0">2</span>
                          <p>Pilih jawaban dengan hati-hati. Gunakan tombol "Ragu-ragu" (Flag) untuk menandai soal yang ingin diperiksa ulang.</p>
                      </div>
                      <div className="flex gap-4 items-start">
                          <span className="bg-slate-800 text-yellow-500 font-bold w-8 h-8 flex items-center justify-center rounded-sm shrink-0">3</span>
                          <p>Dilarang melakukan kecurangan (Pindah Tab/Minimize). Anda memiliki 3 nyawa. Jika habis, Anda akan tereliminasi (Diskualifikasi).</p>
                      </div>
                      <div className="flex gap-4 items-start">
                          <span className="bg-slate-800 text-yellow-500 font-bold w-8 h-8 flex items-center justify-center rounded-sm shrink-0">4</span>
                          <p>Klik tombol "Finish" hanya jika Anda yakin telah menyelesaikan semua soal.</p>
                      </div>
                  </div>

                  <button 
                      onClick={() => setIntroStep(3)}
                      className="w-full py-4 bg-green-600 hover:bg-green-500 text-white font-black text-xl uppercase tracking-[0.2em] shadow-lg transition-transform active:scale-95"
                  >
                      Masuk ke Lobby
                  </button>
              </div>
  ...
  // 1. RESULT SCREEN (Victory/Booyah)
  if (isSubmitted && activeExam) {
      return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
          ...
          <div className="relative bg-black/80 border-2 border-yellow-500 p-1 rounded-3xl shadow-[0_0_50px_rgba(234,179,8,0.3)] animate-bounce-in max-w-lg w-full mx-4">
             <div className="bg-slate-900/90 rounded-[20px] p-8 text-center border border-white/10 overflow-hidden relative">
                ...
                <div className="bg-slate-800/50 p-6 rounded-xl mb-8 border border-slate-700 relative">
                  <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-slate-900 px-4 text-xs font-bold text-yellow-500 uppercase tracking-widest border border-slate-700">
                      Final Score
                  </div>
                ...
                <button 
                    ...
                    className="w-full bg-yellow-500 text-black py-4 rounded-lg font-black text-lg uppercase tracking-widest hover:bg-yellow-400 transition transform hover:scale-105 shadow-lg clip-path-button"
                >
                  Return to Lobby
                </button>
             </div>
    ...
    return (
      <div className="h-screen flex flex-col bg-slate-950 text-white font-sans overflow-hidden relative">
        ...
        {showViolationWarning && (
            <div className="fixed inset-0 z-[70] flex items-center justify-center bg-red-900/40 backdrop-blur-md animate-pulse">
                <div className="bg-black border-4 border-red-600 w-full max-w-lg p-1 relative shadow-[0_0_50px_rgba(220,38,38,0.5)] transform scale-110">
                    <div className="bg-slate-900 p-8 text-center border border-white/10">
                         ...
                         <p className="text-red-400 font-bold text-lg mb-6 uppercase tracking-wide">
                             Dilarang Keluar dari Aplikasi
                         </p>
                         <div className="flex justify-center gap-4 mb-8">
                             {[1, 2, 3].map(i => (
                                 <div key={i} className={`w-12 h-12 flex items-center justify-center border-2 font-black text-xl transform rotate-45 ${i <= violationCount ? 'bg-red-600 border-red-500 text-white' : 'bg-slate-800 border-slate-700 text-slate-600'}`}>
                                     <span className="transform -rotate-45">{i <= violationCount ? 'X' : i}</span>
                                 </div>
                             ))}
                         </div>
                         <p className="text-slate-400 text-sm mb-6">
                             Anda telah melakukan pelanggaran {violationCount}/3. Jika mencapai 3 kali, Anda akan langsung didiskualifikasi!
                         </p>
                         <button 
                            onClick={() => setShowViolationWarning(false)}
                            className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-black uppercase tracking-widest text-xl"
                         >
        ...
        <div className="relative z-20 bg-black/60 backdrop-blur-md border-b border-yellow-600/30 px-4 md:px-8 py-3 flex justify-between items-center shadow-lg">
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="lg:hidden p-2 text-yellow-500 border border-yellow-500 rounded-lg bg-slate-900/50 hover:bg-slate-800 transition"
            >
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
             ...
             <div className="hidden md:flex items-center gap-1 bg-black/40 px-3 py-1.5 border border-red-500/30 rounded">
                 <ShieldIcon size={16} className={violationCount > 0 ? "text-red-500" : "text-green-500"} />
            ...
            <button 
                ...
                className="bg-gradient-to-r from-orange-600 to-red-600 text-white px-3 md:px-6 py-2 hover:from-orange-500 font-black uppercase tracking-wider transform -skew-x-12 flex items-center gap-2 border border-white/20 text-xs md:text-base"
            >
        ...
        <div className="lg:hidden w-full bg-slate-900 border-b border-white/10 z-10 flex items-center">
            <div 
              ref={navScrollRef}
              className="flex-1 flex overflow-x-auto gap-2 p-2 custom-scrollbar scroll-smooth snap-x"
            >
              {activeExam.questions.map((q, idx) => {
                 const hasAns = answers[q.id] !== undefined;
                 const isMarkedDoubt = doubts.has(q.id);
                 ...
                 return (
                   <button
                     ...
                     className={`flex-none w-10 h-10 rounded-lg border-2 flex items-center justify-center font-bold text-sm transition-all snap-center ${bgClass}`}
                   >
                     {idx + 1}
            ...
            <div className="px-2 bg-slate-900 shadow-[-10px_0_10px_rgba(0,0,0,0.5)] z-20 h-full flex items-center border-l border-white/10">
                <span className="text-[10px] text-slate-500 font-mono uppercase text-center block leading-tight">
                  <span className="text-white block font-bold">{currentQuestionIndex + 1}</span>
                  of {activeExam.questions.length}
                </span>
            </div>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden relative z-10">
              ...
              <div className="bg-slate-900/80 backdrop-blur-md p-6 md:p-10 rounded-none border-t-2 border-yellow-500 shadow-2xl min-h-[500px] flex flex-col relative">
                <div className="flex justify-between items-start mb-8 border-b border-slate-700 pb-4">
                  <div className="flex items-center gap-3">
                      <span className="bg-yellow-500 text-black px-3 py-1 text-sm font-black uppercase transform -skew-x-12">
                        <span className="skew-x-12">Question {currentQuestionIndex + 1}</span>
                      </span>
                      {isDoubt && (
                         <span className="bg-orange-600 text-white px-2 py-1 text-xs font-bold uppercase rounded flex items-center gap-1">
                           <Flag size={12} fill="currentColor" /> Ragu-ragu
                         </span>
                      )}
                  </div>
                ...
                {currentQuestion.image && (
                   <div className="w-full mb-8 flex flex-col items-center bg-slate-900/50 p-4 rounded-xl border border-slate-700 relative group">
                      ...
                      <img 
                        ...
                        alt="Lampiran Soal" 
                        className="max-w-full h-auto max-h-[400px] object-contain rounded-lg shadow-lg border border-slate-600 mt-8 md:mt-0" 
                      />
                   </div>
                )}
          ...
          {/* Desktop/Laptop Sidebar (Now Visible on Large Screens) */}
          <div className="w-72 bg-black/80 backdrop-blur border-l border-white/10 p-4 overflow-y-auto hidden lg:block">
            <div className="mb-6 pb-4 border-b border-white/10">
                ...
                {violationCount > 0 && (
                   <div className="bg-red-900/20 border border-red-500/50 p-2 rounded mt-2">
                       <p className="text-xs text-red-400 font-bold uppercase text-center">Security Alert</p>
                       <div className="flex justify-center gap-2 mt-1">
                          {[1,2,3].map(i => (
                              <div key={i} className={`w-2 h-2 rounded-full ${i <= violationCount ? 'bg-red-500 animate-pulse' : 'bg-slate-700'}`}></div>
                          ))}
                       </div>
                   </div>
                )}
          ...
          {isMobileNavOpen && (
            <div className="absolute inset-0 z-50 bg-black/95 backdrop-blur-xl p-6 lg:hidden flex flex-col animate-fade-in">
              <div className="flex justify-between items-center mb-8 border-b border-white/20 pb-4">
                 <h3 className="font-bold text-yellow-500 text-lg uppercase tracking-widest flex items-center gap-2">
                   <Map size={20}/> Tactical Map
                 </h3>
                 <button onClick={() => setIsMobileNavOpen(false)} className="p-2 bg-slate-800 rounded-full text-white hover:bg-red-600 transition">
                   <X size={24}/>
                 </button>
              </div>
  ...
  return (
    <div className="h-full flex flex-col md:flex-row bg-slate-950 text-white font-sans relative overflow-hidden">
        ...
        {/* Compact Navigation (Desktop Left Sidebar / Mobile Bottom Bar) */}
        <div className="md:w-20 md:flex-col flex-row flex md:border-r border-t md:border-t-0 border-white/10 bg-black/60 backdrop-blur-md z-30 fixed bottom-0 w-full md:static md:h-full justify-around md:justify-start items-center md:pt-6 pb-safe">
             ...
             {/* Nav Items */}
             <button 
                onClick={() => { setMenuTab('LOBBY'); setSelectedHistory(null); }}
                className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-all ${menuTab === 'LOBBY' ? 'text-yellow-500 bg-white/10' : 'text-slate-500 hover:text-white'}`}
             >
                <Home size={24} />
                <span className="text-[10px] uppercase font-bold md:hidden">Lobby</span>
             </button>

             <button 
                onClick={() => setMenuTab('CAREER')}
                className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-all mt-0 md:mt-4 ${menuTab === 'CAREER' ? 'text-yellow-500 bg-white/10' : 'text-slate-500 hover:text-white'}`}
             >
                <Trophy size={24} />
                <span className="text-[10px] uppercase font-bold md:hidden">Career</span>
             </button>
             ...
             <button 
                onClick={onLogout}
                className="p-3 rounded-xl flex flex-col items-center gap-1 text-red-500 hover:bg-red-500/10 transition-all mb-0 md:mb-6"
             >
                <LogOut size={24} />
                <span className="text-[10px] uppercase font-bold md:hidden">Exit</span>
             </button>
        </div>
                                        ...
                                        <div key={exam.id} className={`group relative bg-slate-900 border border-slate-700 transition-all duration-300 rounded-xl overflow-hidden shadow-lg ${isTimeValid ? 'hover:border-yellow-500' : 'opacity-75 grayscale'}`}>
                                            ...
                                            <div className="p-5">
                                                <div className="flex justify-between items-start mb-3">
                                                    <span className="bg-emerald-900/40 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest rounded">
                                                        Ranked
                                                    </span>
                                                    <Clock size={14} className="text-slate-500"/>
                                                </div>
                                                
                                                <h3 className="text-lg font-black text-white uppercase italic tracking-wide mb-1 leading-tight group-hover:text-yellow-400 transition-colors">
                                                    {exam.title}
                                                </h3>
                                                ...
                                                {isTimeValid ? (
                                                    <button 
                                                        ...
                                                        className="w-full py-3 bg-yellow-600 hover:bg-yellow-500 text-black font-black uppercase tracking-widest text-xs rounded transition-all active:scale-95 shadow-md flex items-center justify-center gap-2"
                                                    >
                                                    ...
                                                    <button 
                                                        disabled
                                                        className="w-full py-3 bg-slate-800 text-slate-500 font-black uppercase tracking-widest text-xs rounded flex items-center justify-center gap-2 cursor-not-allowed"
                                                    >
                                                        {isEnded ? 'Mission Ended' : 'Coming Soon'}
                                                    </button>
                                                )}
                                            </div>
                                        </div>
                                ...
                                <div className="space-y-3">
                                    {historyData.map((item: any, idx: number) => (
                                        <div 
                                            ...
                                            className={`p-4 border rounded-xl cursor-pointer transition-all relative overflow-hidden ${selectedHistory === item ? 'bg-yellow-500/10 border-yellow-500' : 'bg-slate-900 border-slate-700 hover:bg-slate-800'}`}
                                        >
                                            <div className="flex justify-between items-center relative z-10">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-xs border ${item.score >= 75 ? 'bg-yellow-500 text-black border-yellow-400' : 'bg-slate-700 text-white border-slate-600'}`}>
                                                        {Math.round(item.score)}
                                                    </div>
                                                    <div>
                                                        <h4 className={`font-bold text-sm uppercase ${selectedHistory === item ? 'text-yellow-400' : 'text-slate-200'}`}>
                                                            {item.exam.title}
                                                        </h4>
                                    ...
                                    {historyData.length === 0 && (
                                        <div className="text-center p-8 text-slate-500 italic border border-dashed border-slate-700 rounded-xl">
                                ...
                                <div className="flex-1 bg-slate-900/90 border border-slate-700 rounded-xl overflow-hidden flex flex-col animate-fade-in shadow-2xl relative">
                                     <div className="p-4 border-b border-white/10 flex justify-between items-center bg-black/40">
                                         <h3 className="font-bold text-white uppercase text-sm tracking-widest flex items-center gap-2">
                                            <BarChart2 size={16} className="text-yellow-500"/> Performance Report
                                         </h3>
                                         <button onClick={() => setSelectedHistory(null)} className="lg:hidden p-1 bg-slate-800 rounded-full text-white">
                                             <X size={16}/>
                                         </button>
                                     </div>
                                             ...
                                             {selectedHistory.exam.questions.map((q: Question, qIdx: number) => {
                                                 ...
                                                 return (
                                                     <div key={q.id} className="flex items-start gap-3 p-3 rounded bg-black/20 border border-slate-800/50">
                                                         <div className={`mt-0.5 w-5 h-5 flex-none rounded flex items-center justify-center text-[10px] font-bold ${isCorrect ? 'bg-emerald-500 text-black' : 'bg-red-500 text-white'}`}>
                                                             {isCorrect ? <Check size={12}/> : <X size={12}/>}
                                                         </div>
                                                         <div className="flex-1 min-w-0">
                                                             <p className="text-xs text-slate-300 line-clamp-2 leading-relaxed">{q.text}</p>
                                                             <div className="flex gap-2 mt-1">
                                                                 <span className="text-[9px] text-slate-500 uppercase font-mono">#{qIdx + 1}</span>
                                                                 <span className={`text-[9px] uppercase px-1 rounded ${q.category === QuestionCategory.NUMERASI ? 'text-orange-400 bg-orange-900/20' : 'text-purple-400 bg-purple-900/20'}`}>
                                                                     {q.category || 'General'}
                                                                 </span>
                                                             </div>
                                                         </div>
