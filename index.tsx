...
// --- CONFIGURATION ---
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzcPlmL3RSL3KwIlPrnSwutbnPCzsI4B6BHN42QIGezepe_wlRTLA1touPRh2oX_fw-vg/exec"; 
...
const App = () => {
  ...
  const [examResults, setExamResults] = usePersistentState<ExamResult[]>('cbt_results', []); // NEW: Store Results
  ...
  const stateRef = useRef({ students, questions, exams, packets, examResults });
  useEffect(() => {
      stateRef.current = { students, questions, exams, packets, examResults };
  }, [students, questions, exams, packets, examResults]);
  ...
  const fetchDataFromServer = async () => {
      ...
      try {
          ...
          if(json.status === 'success') {
              ...
              if(json.data.Results?.length) setExamResults(json.data.Results); // Load Results
              if(json.data.Settings) setSchoolSettings({ ...schoolSettings, ...json.data.Settings }); // Load Settings if available
              setIsConnected(true);
  ...
  // Function to save data (Passed to AdminDashboard)
  const saveDataToServer = async () => {
      if(!APPS_SCRIPT_URL) return;
      setIsSyncing(true);
      setSyncError('');

      // CRITICAL: Use data from ref to ensure we send the LATEST state
      const payload = {
          action: 'write',
          data: {
              ...
              Results: stateRef.current.examResults, // Save Results
              Settings: schoolSettings // Save Settings (including passwords)
          }
      };

      try {
          // Use 'no-cors' for Google Apps Script doPost compatibility.
          ...
          // Assume success if no network error (due to opaque response)
          setIsConnected(true);
          console.log("Data sent to server (Background/No-CORS)");
      } catch (err: any) {
  ...
  // Helper for Student Saving Result
  const handleStudentSaveResult = (result: ExamResult) => {
      setExamResults(prev => [...prev, result]);
      // Optional: Auto sync when student finishes?
      // For now, we rely on local storage until Admin syncs or we implement auto-sync
      setTimeout(() => saveDataToServer(), 1000); 
  };
            ...
            <div className="flex-1 h-full overflow-hidden relative">
                {session.role !== Role.STUDENT && (
                  <AdminDashboard
                    ...
                    examResults={examResults} // PASS RESULTS
                    onSyncData={() => setTimeout(saveDataToServer, 500)} // Delay slightly to allow state to settle
                  />
                )}
                {session.role === Role.STUDENT && session.details && (
                  <StudentDashboard
                    ...
                    examResults={examResults} // Pass Results
                    onSaveResult={handleStudentSaveResult} // Pass Save Function
                  />
                )}
            </div>
        </div>
