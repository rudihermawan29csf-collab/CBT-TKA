...
// --- CONFIGURATION ---
// PASTE URL DEPLOYMENT BARU ANDA DI SINI SETELAH LANGKAH 3
...
const App = () => {
  ...
  const fetchDataFromServer = async () => {
      ...
      try {
          ...
          if(json.status === 'success') {
              // Only update if data exists on server
              ...
              // Load Settings if available (and merge to avoid missing keys)
  ...
  const handleStudentSaveResult = (result: ExamResult) => {
      setExamResults(prev => {
          ...
          // Update ref immediately for the pending save
          stateRef.current.examResults = next;
          return next;
      });
      // Delay save slightly to allow state update to propagate
