...
import { User, Lock, Crosshair, Target, AlertTriangle, ChevronRight, ChevronDown, Cloud, CloudOff, RefreshCw, ExternalLink, Save } from 'lucide-react';
...
const App = () => {
  ...
  const fetchDataFromServer = async () => {
      ...
      try {
          ...
          if (text.trim().startsWith('<')) throw new Error("Server Response HTML (Check URL)");
  ...
  // 2. WRITE DATA (POST)
  const saveDataFromServer = async (overrideData?: any) => {
      if(!APPS_SCRIPT_URL) {
          alert("URL Server belum dikonfigurasi di index.tsx");
          return;
      }
      setIsSyncing(true);
      setSyncError('');
      ...
      const payload = {
          ...
          data: {
              ...
              Questions: overrideData?.questions || questions,
              Packets: overrideData?.packets || packets,
              Exams: overrideData?.exams || exams
          }
      };

      try {
          // Use 'no-cors' mode is risky because we can't read response, 
          // but Google Apps Script often requires it for simple POSTs from browser.
          // Ideally, your GAS `doPost` returns JSON with CORS headers.
          // Trying standard POST first.
          const res = await fetch(APPS_SCRIPT_URL, {
              method: 'POST',
              headers: {
                  'Content-Type': 'text/plain;charset=utf-8', // GAS prefers text/plain to avoid preflight options
              },
              body: JSON.stringify(payload)
          });

          // If we can read the response
          if(res.ok) {
              const text = await res.text();
              try {
                  const json = JSON.parse(text);
                 ...
                 // Even if JSON parse fails, if response was OK, assume data sent.
                 console.warn("Write response OK but parse failed", e);
              }
          }
      } catch (err: any) {
          console.error("Upload failed:", err);
          setSyncError("Upload Failed (Check Console)");
      } finally {
          setIsSyncing(false);
      }
  };
    ...
    return (
      <div className="fixed inset-0 w-full h-full flex flex-col items-center justify-center p-4 bg-slate-950 overflow-hidden font-sans text-white">
        ...
        <div className="absolute top-4 right-4 flex flex-col items-end gap-2 z-50">
            {isSyncing ? (
                <div className="bg-blue-900/50 text-blue-300 px-3 py-1 rounded-full text-xs font-bold border border-blue-500/30 flex items-center gap-2 shadow-lg animate-pulse">
  ...
  // --- Main Application View (FIXED LAYOUT) ---
  return (
    // CHANGE: h-screen to fixed inset-0 to force viewport clipping
            ...
            {session.role === Role.ADMIN && (
              <div className="absolute top-4 right-8 z-50 flex items-center gap-2">
                 {isSyncing ? (
                     ...
                     <button 
                        onClick={() => saveDataFromServer()} 
                        className="bg-slate-800 hover:bg-green-600 px-3 py-1 rounded text-xs font-bold text-slate-300 hover:text-white border border-slate-600 shadow-lg flex items-center gap-2 transition-all"
                        title="Upload data baru ke server"
                     >
